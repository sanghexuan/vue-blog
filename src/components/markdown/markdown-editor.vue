...
export default {
  ...
  mounted() {
    ...
    if (
      !(
        typeof _this.externalLink === "object" &&
        typeof _this.externalLink["markdown_css"] === "function"
      )
    ) {
      // 没有外部文件要来接管markdown样式，可以更改markdown样式。
      _this.codeStyleChange(_this.codeStyle, true);
    }
  },
  ...
  methods: {
    ...
    initExternalFuc() {
      ...
      var _type_ = typeof _this.externalLink;
      var _is_object = _type_ === "object";
      var _is_boolean = _type_ === "boolean";
      for (var i = 0; i < _external_.length; i++) {
        if (
          (_is_boolean && !_this.externalLink) ||
          (_is_object && _this.externalLink[_external_[i]] === false)
        ) {
          _this.p_external_link[_external_[i]] = false;
        } else if (
          _is_object &&
          typeof _this.externalLink[_external_[i]] === "function"
        ) {
          _this.p_external_link[_external_[i]] =
            _this.externalLink[_external_[i]];
        } else {
          _this.p_external_link[_external_[i]] =
            _this.s_external_link[_external_[i]];
        }
      }
    },
    textAreaFocus() {
    ...
    $imgAdd(pos, $file, isinsert) {
      ...
      this.__oFReader.onload = function (oFREvent) {
        _this.markdownIt.image_add(pos, oFREvent.target.result);
        ...
        if (isinsert === true) {
          ...
          _this.insertText(_this.getTextareaDom(), {
            prefix: "![" + $file._name + "](" + pos + ")",
            subfix: "",
            str: "",
          });
          _this.$nextTick(function () {
            _this.$emit("imgAdd", pos, $file);
          });
        }
      };
      if ($file) {
    ...
    $imgUpdateByUrl(pos, url) {
      ...
      this.$nextTick(function () {
        _this.d_render = this.markdownIt.render(this.d_value);
      });
    },
    $imgAddByUrl(pos, url) {
    ...
    getNavigation(_this, full) {
      return getNavigation(_this, full);
    },
    // @event
    ...
    initLanguage() {
      ...
      var _this = this;
      _this.$render(CONFIG[`help_${lang}`], function (res) {
        _this.d_help = res;
      });
    ...
    iRender(toggleChange) {
      var _this = this;
      this.$render(_this.d_value, function (res) {
        // render
        _this.d_render = res;
        ...
        // 塞入编辑记录数组
        if (_this.d_value === _this.d_history[_this.d_history_index]) return;
        window.clearTimeout(_this.currentTimeout);
        _this.currentTimeout = setTimeout(() => {
          _this.saveHistory();
        }, 500);
      });
    },
